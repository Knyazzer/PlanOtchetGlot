...
## СЛЕДУЮЩИЙ ШАГ — отображение заявок в календаре кабинета (MonthStrip) + жизненный цикл задач на днях отсутствия

> **✅ РЕАЛИЗОВАНО 2026-08-31** (ветка knyazzer, коммиты 00c37e8 / f9a1774 / a271874): С1 pending пунктиром + approved подпись (MonthStrip); С2 задачи на днях отсутствия — `isDayActive`/`canAddTask` пускают отпуск/больничный/отгул (дата ≤ сегодня) без «начала дня»; С3 просрочка — общий `isOverdue()` в lib/taskWindow.ts (дедлайн прошёл ИЛИ inprogress && startDate<today). Осталось: overdue-подсветка на строке задачи. Ниже — исходная спека.
