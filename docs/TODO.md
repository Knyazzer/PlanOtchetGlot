...
- [ ] **Уведомления — механика в разработке** — `NotificationBell` заглушка. Нужно спроектировать: читаемость глобальных уведомлений отдельно для каждого пользователя (таблица `user_notification_reads`?), механика для admin vs employee, правильная логика `read-all`. Текущий `read-all` помечает прочитанными и `userId = me.id`, и `userId = null` (глобальные). Любой пользователь сбрасывает счётчик глобальных уведомлений у всех остальных.
...
- [ ] **`docs/04-database-schema.md`** — требует проверки актуальности: добавлены модели `MatrixTemplate`, `ProjectMember`, `SheetConfig`, новые enum'ы.
- [ ] **`docs/05-architecture.md`** — требует проверки после добавления Google Drive-интеграции, DatabasePage и внутренних матриц.
- [ ] **`DELETE /project-members/:id` не возвращает 404** — при удалении несуществующего ID возвращает `{ ok: true }` без ошибки. Незначительно, но может маскировать баги.
- [ ] **Нейминг `sheet_url` для Drive folder** — в таблице `sheet_configs` колонка `sheet_url` хранит ID папки Google Drive для ключа `drive_folder`. Работает, но вводит в заблуждение при чтении схемы.
