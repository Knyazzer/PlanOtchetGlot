...
export async function dayEntriesRoutes(app: FastifyInstance) {
  ...
  app.put('/', { preHandler: authenticate }, async (req, reply) => {
    ...
    // ── Будущий день (не доверяем клиенту): вести/начинать/править рабочий день можно только сегодня
    //    или задним числом. Иначе прямым API можно нафабриковать «отработанные» часы и inprogress-задачи
    //    наперёд (искажает Свод/Аналитику) и сломать дневную цепочку (будущий день становится «дыркой»).
    //    Клиентский isFuture (DayFillCard) — только UX; правило живёт здесь. Override — мастер-админ.
    //    Планирование задач в backlog на будущее НЕ через этот путь (POST /tasks без активного дня).
    if (!(req as any).user?.isAdmin) {
      const now = new Date()
      const p2 = (n: number) => String(n).padStart(2, '0')
      const todayStr = `${now.getFullYear()}-${p2(now.getMonth() + 1)}-${p2(now.getDate())}`
      if (date > todayStr) {
        return reply.code(400).send({ error: 'Этот день ещё не наступил — рабочий день ведётся только сегодня или задним числом' })
      }
    }
