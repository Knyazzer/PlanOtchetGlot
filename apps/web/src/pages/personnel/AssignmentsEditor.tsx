...
const inp: React.CSSProperties = {
  ...
  borderRadius: 8, color: 'var(--text-1)', fontSize: 14, padding: '8px 10px',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <button onClick={() => save.mutate(undefined)} disabled={!valid || save.isPending}
        style={{ background: valid ? 'var(--accent, #2563eb)' : 'var(--surface-2)', color: valid ? '#fff' : 'var(--text-muted)', border: 'none', borderRadius: 8, padding: 9, cursor: valid && !save.isPending ? 'pointer' : 'default', fontSize: 14, fontWeight: 600 }}>
    ...
    >
      <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, width: 360, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={onCancel} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 14px', color: 'var(--text-2)', cursor: 'pointer', fontSize: 14 }}>Отмена</button>
          <button onClick={onReplace} disabled={pending} style={{ background: 'var(--danger, #dc2626)', border: 'none', borderRadius: 8, padding: '8px 14px', color: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>{pending ? '…' : 'Заменить'}</button>
        </div>
      </div>
    </div>
  )
