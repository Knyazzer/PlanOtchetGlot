...
| `/requests` | `routes/requests.ts` | заявки (отпуск/больничный/отгул): GET /types, GET ?scope=mine\|inbox, POST, PATCH /:id/decision (согласующий/админ), PATCH /:id/cancel (автор), GET /:id/document (docx-заявление для одобренного отпуска, `services/requestDocx.ts`); согласующий = рук. отдела→директор департамента→админ. Спека — docs/REQUESTS-MODULE.md |
