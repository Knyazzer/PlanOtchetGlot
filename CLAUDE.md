...
| `/task-templates` | `routes/task-templates.ts` | шаблоны задач (пресеты) — «штампы» для быстрого создания обычной Task: GET/POST/PATCH/DELETE только СВОИ (403 на чужой). Пресет = не задача; инстанцирование (клик по шаблону) идёт через обычный `POST /tasks` с предзаполнением (те же гейты: сегодня-активный→inprogress, будущий→backlog). UI — правая панель `TaskTemplatesPanel` + кнопка «Шаблонная задача» в кабинете |
