...
| `/strategic-goals` | `routes/strategic-goals.ts` | стратегические цели (квартал/год), каскад департамент→отдел: GET ?periodKey (охват своего департамента; admin — все), POST/PATCH/DELETE (director/head/admin, открытый период), PATCH /:id/close (статус+итог, закрытие вручную). Спека — docs/STRATEGIC-GOALS.md. Task.goalId/Track.goalId — привязка задач/треков к цели |
