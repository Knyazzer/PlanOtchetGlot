...
--
-- ВАЖНО (2026-07-11): миграция переименована из 20260609_sso_multischema →
-- 20260609100000_sso_multischema, чтобы идти ДО 20260609213923_password_management,
-- который правит nexus.users (иначе накат с нуля падал: relation "nexus.users" does not exist).
-- Всё тело обёрнуто в идемпотентный guard: на проде, где перенос уже выполнен старой миграцией,
-- повторный прогон (contaner CMD `migrate deploy` при старте видит новое имя как непринятое) —
-- безопасный no-op. migrate resolve не требуется.
-- ============================================================

DO $sso_multischema$
BEGIN
  IF to_regclass('nexus.users') IS NOT NULL THEN
    RAISE NOTICE 'sso_multischema: nexus.users уже существует — миграция уже применена, пропускаем';
    RETURN;
  END IF;
